...
rke_guided_sampler = RKEGuidedSampling(
    ...
    model_name="audioldm", #maybe change to none but does not make sense 
    model=pipe
)

# 4. GUIDANCE PARAMETERS
guidance_kwargs = dict(
    num_inference_steps=200,      # <--- 200 is best for V2
    guidance_scale=7.5,           # <--- Added to force prompt adherence in V2
