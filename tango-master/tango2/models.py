...
# Helper function for older diffusers API compatibility
def randn_tensor(shape, generator=None, device=None, dtype=None):
    """Create a random tensor similar to diffusers.utils.randn_tensor"""
...
class AudioDiffusion(nn.Module):
    def __init__(
        ...
        assert (unet_model_name is not None or 
                unet_model_config_path is not None or 
                unet_model_config is not None), "Either UNet pretrain model name, config file path, or config dict is required"
        ...
        if unet_model_config:
            self.unet = UNet2DConditionModel(**unet_model_config)
            self.set_from = "random"
            print("UNet initialized from config dict.")
        elif unet_model_config_path:
