...
def cov_rff2_joint(x, feature_dim, std, split_dim, batchsize=16, presign_omeaga=None, normalise=True):
    """
    split_dim: Integer specifying the dimension size of the first modality (e.g., 512 for CLAP, 768 for larger embeddings).
    """
    assert len(x.shape) == 2 # [B, dim]
    ...
    # Use dynamic split_dim instead of hardcoded 780
    y[:, split_dim:] = -y[:, split_dim:].clone()
    ...
    batched_rff = batched_rff.unsqueeze(2) # [B, 4 * feature_dim, 1]

    cov = torch.zeros((4 * feature_dim, 4 * feature_dim), device=x.device)
    batch_num = (x.shape[0] // batchsize) + 1
    i = 0
    for batchidx in tqdm(range(batch_num)):
        batched_rff_slice = batched_rff[batchidx*batchsize:min((batchidx+1)*batchsize, batched_rff.shape[0])] # [mini_B, 4 * feature_dim, 1]
        cov += torch.bmm(batched_rff_slice, batched_rff_slice.transpose(1, 2)).sum(dim=0)
        i += batched_rff_slice.shape[0]
    cov /= x.shape[0]
    assert i == x.shape[0]
...
def cov_rff2_joint_v2(x, feature_dim, std, split_dim, batchsize=16, presign_omeaga=None, normalise=True):
    """
    split_dim: Integer specifying the dimension size of the first modality.
    """
    assert len(x.shape) == 2 # [B, dim]
    ...
    # Dynamically split based on split_dim
    omegas_mod1, omegas_mod2 = omegas[:split_dim], omegas[split_dim:]
    mod1, mod2 = x[:, :split_dim], x[:, split_dim:]
    
    product_mod1 = mod1 @ omegas_mod1
    product_mod2 = mod2 @ omegas_mod2
    
    batched_rff_cos = torch.cos(mod1 @ omegas_mod1 + mod2 @ omegas_mod2) # [B, feature_dim]
    batched_rff_sin = torch.sin(mod1 @ omegas_mod1 + mod2 @ omegas_mod2) # [B, feature_dim]
    batched_rff_cos_negative = torch.cos(mod1 @ omegas_mod1 - mod2 @ omegas_mod2) # [B, feature_dim]
    batched_rff_sin_negative = torch.sin(mod1 @ omegas_mod1 - mod2 @ omegas_mod2) # [B, feature_dim]

    batched_rff = torch.cat([torch.cos(product_mod1) * torch.cos(product_mod2),
                             torch.cos(product_mod1) * torch.sin(product_mod2),
                             torch.sin(product_mod1) * torch.cos(product_mod2),
                             torch.sin(product_mod1) * torch.sin(product_mod2)], dim=1) / np.sqrt(feature_dim) # [B, 4 * feature_dim]
    ...
    def compute_joint_entropy(self, X, Y, order, compute_kernel=False, algorithm='kernel', feature_dim=3000):
        ...
        if algorithm == 'rff':
            ...
            cov_joint, self.omegas_x, self.omegas_y, _ = joint_cov_rff(x=X, y=Y, feature_dim=feature_dim, std_x=sigma_x,
                                                                       ...
                                                                       omegas_x=self.omegas_x, omegas_y=self.omegas_y)
