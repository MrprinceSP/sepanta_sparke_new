...
logger = None
try:
    from .stable_diffusion_xl import make_logger
...
class RKEGuidedSampling:
    ...
    def get_rank(self, M, M_text=None, feature_m=None, feature_t=None, kernel='cosine', sigma_image=None,
        ...
        if self.algorithm == 'rke':  # RKE
            ...
            # 1. Get eigenvalues directly (much faster than SVD)
            eigenvalues = torch.linalg.eigvalsh(XoY.to(torch.float32))
            eigenvalues= torch.abs(eigenvalues)  # Ensure non-negative eigenvalues``
            
            # 2. Clamp to prevent log(0) -> NaN crashes
            p = torch.clamp(eigenvalues, min=1e-12)
            
            # 3. Normalize into a probability distribution
            p = p / p.sum()
            
            # 4. Calculate entropy and Vendi rank
        ...
        elif self.algorithm == 'cond-vendi-FKEA':
            # --- FKEA Implementation using Random Fourier Features ---
            if feature_m is None or feature_t is None:
                raise ValueError("algorithm='cond-vendi' requires raw feature tensors 'feature_m' and 'feature_t'.")

            device = feature_m.device
            B, Dx = feature_m.shape
            _, Dy = feature_t.shape

            # Normalize embeddings for cosine kernel mapping
            X = F.normalize(feature_m, p=2, dim=1).to(torch.float32)
            Y = F.normalize(feature_t, p=2, dim=1).to(torch.float32)

            sig_x = sigma_image if sigma_image is not None else 1.0
            sig_y = sigma_text if sigma_text is not None else 1.0

            # 1. Random Fourier Features for Modality 1 (Images)
            
            proj_x = torch.mm(X, omegas_x)
            phi_x = (1.0 / math.sqrt(rff_dim)) * torch.cat([torch.cos(proj_x), torch.sin(proj_x)], dim=1)

            # 2. Random Fourier Features for Modality 2 (Text)
            proj_y = torch.mm(Y, omegas_y)
            phi_y = (1.0 / math.sqrt(rff_dim)) * torch.cat([torch.cos(proj_y), torch.sin(proj_y)], dim=1)

            # 3. Multimodal Fourier Feature Map (Hadamard product)
            phi_joint = phi_x * phi_y  # Shape: [B, 2 * rff_dim]

            # 4. Compact Proxy Covariance Matrix (2*rff_dim x 2*rff_dim)
            cov_matrix = torch.mm(phi_joint.T, phi_joint) / B

            # 5. Eigendecomposition & Eigenvalue Normalization
            eigenvalues = torch.linalg.eigvalsh(cov_matrix)
            eigenvalues = torch.abs(eigenvalues)  # Ensure non-negative eigenvalues
            #top values only, instead of the 2000 of them
            #values, indices= torch.topk(eigenvalues, k=min(B,100))
            #values= torch.abs(values)
            #eigenvalues = torch.topk(eigenvalues, k=80) # Keep top eigenvalues
            p = torch.clamp(eigenvalues, min=1e-10)
            p = p / p.sum()

            # 6. Shannon Entropy & Conditional Vendi Rank Calculation
            entropy = -torch.sum(p * torch.log(p))
            rank = torch.exp(entropy)
            ...
            return torch.exp(entropy)
        elif self.algorithm == 'cond-vendi-CSA':
            # XoY: (n, n) positive semi-definite similarity matrix
            XoY= M * M_text  # Hadamard product
            n = XoY.shape[0]
            k = min(n, 20)
            c = min(n, 30)
            p = 10
            # Derive device directly from the tensor
            device = XoY.device if 'XoY' in locals() else M.device

            # 1. Subsample landmark columns (always including index n - 1)
            if n <= c:
                sample_indices = torch.arange(n, device=device)
            else:
                # Pick (c - 1) indices from range [0, n - 2]
                other_indices = torch.randperm(n - 1, device=device)[:c - 1]
                last_index = torch.tensor([n - 1], device=device)
                sample_indices = torch.cat([other_indices, last_index])

            C = XoY[:, sample_indices]#A_:1
            W = C[sample_indices, :]#A_11

            # Landmark submatrix
            C = XoY[:, sample_indices]
            W = C[sample_indices, :]

            # Moore-Penrose pseudoinverse of the c x c landmark matrix
            W_pinv = torch.linalg.pinv(W, rtol=1e-7)

            # c x c reduced matrix
            CTC = C.T @ C
            Z = CTC @ W_pinv
            ...
            eigvals = torch.abs(eigvals)  # Ensure non-negative eigenvalues

            p = torch.clamp(
                torch.sort(eigvals, descending=True).values,
                min=1e-12
            )

            # 6. Normalize using the trace of the original N x N matrix
            p = p / p.sum()

            # 7. Truncated von Neumann entropy
            entropy = -torch.sum(p * torch.log(p))
            rank = torch.exp(entropy)
            #rank = rank* k
            return rank
    ...
    def cond_fn(
        ...
        if 'cond' in self.algorithm:
            ...
            if prompt_embeds is not None:
                features_text = prompt_embeds.mean(dim=1).to(torch.float32)
            elif 'prompt_embeds' in kwargs and kwargs['prompt_embeds'] is not None:
        ...
        if self.F_M is not None:  # and index < 45:
            ...
            if 'cond' in self.algorithm:
                ...
                rank_fake = self.get_rank(M=M_ / M_.shape[0], M_text=T_ / T_.shape[0], feature_m=F_, feature_t=F_T_, kernel=self.kernel, sigma_image=self.sigma[0], sigma_text=self.sigma[1],omegas_x=omegas_x, omegas_y=omegas_y)
            else:
                rank_fake = self.get_rank(M=M_ / M_.shape[0], feature_m=F_, kernel=self.kernel, sigma_image=self.sigma[0],omegas_x=omegas_x, omegas_y=omegas_y)
            ...
            if beta != 0 and F_M_real is not None:
                ...
                if 'cond' in self.algorithm:
                    ...
                    rank_real = self.get_rank(M_real_ / M_real_.shape[0], M_text=T_real_ / T_real_.shape[0],
                                              ...
                                              sigma_image=self.sigma[0], sigma_text=self.sigma[1],omegas_x=omegas_x, omegas_y=omegas_y)
                else:
                    rank_real = self.get_rank(M_real_ / M_real_.shape[0], feature_m=F_real_, kernel=self.kernel,
                                              sigma_image=self.sigma[0],omegas_x=omegas_x, omegas_y=omegas_y)
            else:
                rank_real = 0.0
        ...
        if timestep.item() in [20, 1, 0]:  # TODO: this is bug when samples are already generated or not!!!!!, this is but to compare with 82, but check it later, 20 for sd and 1 for sdxl
